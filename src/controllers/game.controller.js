...
export const startGame = async (req, res) => {
 try {
  ...
  res.json({ id: sessionResult.rows[0]?.id, user_id: userId, title: locationResult.rows[0]?.title});
 } catch (err) {
...
export const submitGuess = async (req, res) => {
 ...
 try {
  ...
  const { user_id, is_completed, is_active, latitude, longitude, location_id, point, group_id, name } = session;
  ...
  } else if( distance - WIN_DISTANCE_KM < 50 && distance - WIN_DISTANCE_KM > 20 ) {
